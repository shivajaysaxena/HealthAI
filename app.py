...
from modules.rag_medical import MedicalRAG
from modules.conversation import MedicalConversation
from modules.medical_documents import process_medical_document, create_medical_context
...
if "uploaded_documents" not in st.session_state:
    st.session_state.uploaded_documents = []  # Store processed documents
...
def consultation_page():
    ...
    with st.expander("📄 Upload Medical Documents (Optional)", expanded=False):
        st.write("**Upload prescriptions, test reports, or medical records**")
        st.info("💡 Tip: Works with both printed AND handwritten documents!")
        ...
        uploaded_files = st.file_uploader(
            ...
            key="doc_uploader",
            help="Supports handwritten prescriptions with 70%+ accuracy"
        )
        
        if uploaded_files:
            if st.button("🔍 Process Documents", use_container_width=True):
                with st.spinner("Processing medical documents with advanced OCR..."):
                    ...
                    for uploaded_file in uploaded_files:
                        ...
                        progress_text.text("Step 1/3: Image enhancement...")
                        
                        doc_info = process_medical_document(uploaded_file)
                        ...
                        if doc_info:
                            ...
                            is_handwritten = doc_info.get('is_handwritten', False)
                            
                            if confidence >= 80:
                                st.success(f"✅ {uploaded_file.name} - {confidence}% confidence {'✍️ (Handwritten)' if is_handwritten else '📄 (Printed)'}")
                            elif confidence >= 60:
                                st.warning(f"⚠️ {uploaded_file.name} - {confidence}% confidence {'✍️ (Handwritten)' if is_handwritten else ''} - Review carefully")
                            else:
                                st.error(f"❌ {uploaded_file.name} - {confidence}% confidence - Low quality, please re-upload clearer image")
                            
                            # Show extracted info
                            with st.expander(f"📋 View extracted info from {uploaded_file.name}"):
                                st.markdown(f"**Confidence:** {confidence}%")
                                st.markdown(f"**Type:** {'Handwritten ✍️' if is_handwritten else 'Printed 📄'}")
                                st.markdown("---")
                                st.markdown(doc_info['parsed_info'])
                        else:
                            st.error(f"❌ Failed to process {uploaded_file.name} - Try clearer image")
                        
                        progress_text.empty()
                    
                    # Add to conversation context
                    if st.session_state.uploaded_documents:
                        medical_context = create_medical_context(st.session_state.uploaded_documents)
                        st.session_state.conversation.add_medical_documents(medical_context)
                        
                        avg_confidence = sum(d.get('confidence', 0) for d in st.session_state.uploaded_documents) / len(st.session_state.uploaded_documents)
                        st.success(f"✅ {len(st.session_state.uploaded_documents)} document(s) processed | Avg confidence: {avg_confidence:.0f}%")
        
        # Show currently loaded documents
        if st.session_state.uploaded_documents:
            st.info(f"📋 {len(st.session_state.uploaded_documents)} document(s) loaded for consultation")
            
            # Show summary
            for doc in st.session_state.uploaded_documents:
                conf_emoji = "✅" if doc.get('confidence', 0) >= 70 else "⚠️"
                type_emoji = "✍️" if doc.get('is_handwritten', False) else "📄"
                st.caption(f"{conf_emoji} {type_emoji} {doc['file_name']} - {doc.get('confidence', 0)}%")
            
            if st.button("🗑️ Clear All Documents"):
                st.session_state.uploaded_documents = []
                st.session_state.conversation.collected_info["medical_documents"] = []
                st.rerun()
...
def extract_analysis_for_speech(analysis_text):
    """Extract concise speech-friendly content (under 480 chars)"""
    ...
    for i, line in enumerate(lines):
        if len(line) < 10 or 'disclaimer' in line.lower() or '---' in line:
            continue
            
        # Categorize content
        if i < 3 and not any(word in line.lower() for word in ['possible', 'condition', 'specialist', 'care']):
            overview.append(line)
        elif 'condition' in line.lower() or any(word in line.lower() for word in ['headache', 'fever', 'pain', 'infection']):
            conditions.append(line)
        elif 'specialist' in line.lower() or 'doctor' in line.lower() or 'physician' in line.lower():
            specialist.append(line)
        elif 'care' in line.lower() or 'advice' in line.lower():
            care.append(line)
    
    # Build speech (stay under 450 chars for safety)
    max_chars = 450
    
    # Start with overview (1 sentence max)
    if overview:
        speech_parts.append(overview[0])
    
    # Add main condition (1 only)
    if conditions:
        speech_parts.append("Possible condition: " + conditions[0].split('.')[0])
    
    # Add specialist
    if specialist:
        speech_parts.append("Consult: " + specialist[0].split('.')[0])
    
    # Add 1-2 care tips
    if care:
        speech_parts.append(care[0].split('.')[0])
    
    speech_text = ". ".join(speech_parts)
    
    # If too long, truncate
    if len(speech_text) > max_chars:
        speech_text = speech_text[:max_chars-50].rsplit('.', 1)[0]
    
    # Add closing
    speech_text += ". Please consult a doctor for proper diagnosis."
    
    return speech_text[:480]  # Hard limit
